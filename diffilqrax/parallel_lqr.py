...
##the associative operator is the conditional value function, i.e min over x

##init states = init trajectory? not sure about that 
def _init_associative_value_functions(params, Xs_init):
    ##this should initialize the associative value functions
    #Ak,k+1 = Fk
    #bk,k+1 = ck
    #Ck,k+1 = LkUk^{-1}L_k^T
    #eta_k,k_1 = H_k^TX_kr_k
    #J_k,k_1 = H_k^TX_kH_k
    #Vk,k+1 = max_\lambda 0.5*x_k^T@J_k,k+1@x_k -x_k^Teta_k - 0.5*\lambda^TCk,k+1\lambda^T 
    #-\lambda^T*(x_k+1 - Ax_k - b_k,k_1)
    ##if we assume C is invertible then 
    ##V_k, k+1 = 0.5*x_k^T@J_k,k+1@x_k -x_k^Teta_k + 0.5*(x_k+1 - Ax_k - b_k,k_1)^TCk,k+1^{-1}(x_k+1 - Ax_k - b_k,k_1)
    #return A, b, C, eta, J, V
    return None
    
def parallel_lqr(params, Xs_init):
    """A parallel version of the lgssm smoothing algorithm.
    See S. Särkkä and Á. F. García-Fernández (2023)
    """
    # filtered_xs = init_xs(params, emissions)
    # filtered_means = filtered_posterior["filtered_means"]
    # filtered_covs = filtered_posterior["filtered_covariances"]
    initial_elements = _init_associative_value_functions(params, Xs_init)

    @vmap
    def value_combination_operator(elem1, elem2):
        A1, b1, C1, eta1, J1, V1, x1 = elem1
        A2, b2, C2, eta2, J2, V2, x2 = elem2
        n = A.shape[0]
        A = A2@jnp.linalg.linsolve((jnp.eye(n) + C2@J1), A1)
        b = A2@jnp.linalg.linsolve((jnp.eye(n) + C2@J1), b1 + C1@eta2) + b2
        C = A2@jnp.linalg.linsolve((jnp.eye(n) + C2@J1), C1@A2.T) + C2
        eta = A1.T@jnp.linalg.linsolve((jnp.eye(n) + J2@C1), eta2 - J1@b2) + eta1
        J = A1.T@jnp.linalg.linsolve((jnp.eye(n) + J2@C1),  J1@A2) + J1
        V = 0.5*x1.T@J@x1 -x1.T@eta + 0.5*(x2 - A@x1 - b).T@jnp.linalg.linsolve(C,(x2 - A@x1 - b))
        return A, b , C, eta, J, V

    A, b , C, eta, J, V = lax.associative_scan(
                                                value_combination_operator, initial_elements, reverse=True
                                                )
    return A, b , C, eta, J, V
